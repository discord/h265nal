...
find src webrtc -type f \( -name '*.cc' -o -name '*.h' \) -print0 | xargs -0 sed -i .bak s/rtc_base/h265nal_base/
...
find src -type f \( -name '*.cc' -o -name '*.h' \) -print0 | xargs -0 sed -i .bak 's/rtc::/h265nal_base::/g'
find webrtc src -type f -name '*.bak' -exec rm {} \;
